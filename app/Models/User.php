...
class User extends Authenticatable
{
                ...
                if ($user[0]->EstadoBloqueo>0){
                    if(isset($user[0]->usrPassword) && Hash::check($data['usrPassword'],$user[0]->usrPassword)){
                            ...
                            if (Auth::check()){
                                ...
                                $usuario = Auth::user();
                                $perfiles = DB::table('v_perfiles_usuarios')->where('idUser',$usuario->idUser)->where('activoPerfil',1)->get();
    ...
    public function mostrarPanel($idPerfil, $des_perfil){
        ...
        switch ($idPerfil) {
            ...
            case 3:
                $result['v_detalle']= DB::table('v_proveedores')->where('idUser',$usuario->idUser)->get();
                $result['idClienteUsuario'] = 0;
    ...
    public function listUsuario(){
        ...
        switch ($idperfil) {
            ...
            case 2:
                $result = DB::table('v_clientes_tienen_usuarios')->where('IdCliente',$p['v_detalle'][0]->IdCliente)->get();
                break;

            case 3:
                $result = DB::table('v_proveedores_tienen_usuarios')->where('IdProveedor',$p['v_detalle'][0]->IdProveedor)->get();
                break;
        }
    ...
    public function perfilesDisponibles(){
        $idUser = Auth::id();
        $result = DB::table('v_perfiles_usuarios')->where('idUser',$idUser)->where('activoPerfil',1)->orderBy('idPerfil')->get();  
    ...
    public function listPerfilesAdministrador($idUser){
        $result = DB::table('v_perfiles_usuarios')->where('idUser',$idUser)->orderBy('idPerfil')->get();
    ...
    public function resetIntentosFallidos($idUser){
        if(isset($idUser)){
            ...
            $result=DB::table('usuarios_bloqueos')->where('idUser', $idUser)->update($values);

            return '{"code":"200","des_code":"Usuario Desbloqueado con exito"}';
        }

        return '{"code":"200","des_code":"Sin información de Usuario"}';
    }
    ...
    public function activarUsuario($datos){
        ...
        return DB::table('usuarios')->where('idUser', $datos['idUser'])->update($values);
    ...
    public function activarPerfil($datos){
        ...
        return DB::table('usuarios_perfiles')->where('idUsuarioPerfil', $datos['idUsuarioPerfil'])->update($values);
    }
    ...
    public function actualizarDatos($data){
        try{
            ...
            $result=DB::table('usuarios')->where('idUser', $data['idUser'])->update($values);
            DB::commit();
    ...
    public function actualizarFoto($idUser,$rutadelaimagen){
        ...
        try{
            DB::beginTransaction();
            $result=DB::table('usuarios')->where('idUser', $idUser)->update(['usrUrlimage' => $rutadelaimagen ]);
            DB::commit();
    ...
    public function eliminarFoto($datos){
        try{
            DB::beginTransaction();
            $result=DB::table('usuarios')->where('idUser', $datos['idUser'])->update(['usrUrlimage' => null ]);
            DB::commit();
    ...
    public function cambiarPassword($datos){
        $count = DB::table('v_usuarios')->where('usrEmail', $datos['email'])->count();
        ...
        try{
            DB::beginTransaction();
            $result=DB::table('usuarios')->where('usrEmail', $datos['email'])->update(['usrPassword' => $Password ]);
            DB::commit();
        ...
        $usrNombreFull = DB::table('v_usuarios')->where('usrEmail',$datos['email'])->value('usrNombreFull');
